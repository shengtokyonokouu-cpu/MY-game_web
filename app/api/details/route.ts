...
export async function GET(request: Request) {
  const url = new URL(request.url); const title = url.searchParams.get("title")?.trim() || ""; const requestedLanguage = url.searchParams.get("lang"); const lang = requestedLanguage === "zh" || requestedLanguage === "ja" ? requestedLanguage : "en";
