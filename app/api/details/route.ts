...
export async function GET(request: Request) {
  ...
  if (!title || title.length > 200) return Response.json({ error: "Invalid title" }, { status: 400 });
  try { return await cached(`details-${lang}-${title}`, 3600, async () => {
    ...
    const page = await response.json() as { extract?: string; type?: string; content_urls?: { desktop?: { page?: string } } };
    if (page.type === "disambiguation") return Response.json({ error: "Ambiguous title" }, { status: 404 });
