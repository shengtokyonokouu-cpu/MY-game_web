...
import { matchesQuery, type CatalogGame } from "../lib/catalog";
import { identityNames, searchTokensMatch } from "../lib/game-names";

export function LiveNewsView({ query, catalog, onOpen }: { query: string; catalog: CatalogGame[]; onOpen: (game: CatalogGame) => void }) {
  const [feed, setFeed] = useState<NewsFeed | null>(null);
  const [state, setState] = useState("loading"); const [filter, setFilter] = useState("all"); const [source, setSource] = useState("all"); const [language, setLanguage] = useState("all"); const [page, setPage] = useState(1); const [archive, setArchive] = useState(false);
  ...
  const queryAliases = useMemo(() => query.trim().length > 1 ? catalog.filter((game) => matchesQuery(game, query)).slice(0, 8).flatMap((game) => [...identityNames(game), ...(game.searchTerms || [])]).filter((name) => name.length > 2) : [], [catalog, query]);
  const items = useMemo(() => (feed?.items || []).filter((item) => (language === "all" || item.language === language) && (source === "all" || item.sourceId === source) && (filter === "all" || filter === item.sourceKind || filter === item.topic) && [query, ...queryAliases].some((term) => searchTokensMatch(`${item.title} ${item.excerpt} ${item.sourceName}`, term))), [feed, filter, source, query, language, queryAliases]);
  ...
  return <><div className="page-heading"><div><p className="eyebrow">LIVE GAME NEWS</p><h1>游戏新闻</h1><p>聚合中日英官方公告与媒体报道，每 5 分钟检查更新。</p></div><button className="button" onClick={() => void refresh()} disabled={state === "loading"}><Icon name="refresh" className={state === "loading" ? "spin" : ""}/>{state === "loading" ? "正在更新…" : "刷新新闻"}</button></div>
    ...
    <div className="news-toolbar"><label>语言 <select value={language} onChange={(e) => { setLanguage(e.target.value); setPage(1); }}><option value="all">中 / 日 / 英</option>{["中文", "日文", "英文"].map((item) => <option key={item}>{item}</option>)}</select></label><label>来源 <select value={source} onChange={(e) => { setSource(e.target.value); setPage(1); }}><option value="all">全部来源</option>{feed?.sources.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><span>{items.length} 条 · 标题保留原文，全文请前往来源网站</span></div>
