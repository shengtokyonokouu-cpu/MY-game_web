...
export const newsSources: NewsSource[] = [
  { id: "playstation", name: "PlayStation Blog", url: "https://blog.playstation.com/feed/", site: "https://blog.playstation.com", kind: "official", language: "英文" },
  { id: "xbox", name: "Xbox Wire", url: "https://news.xbox.com/en-us/feed/", site: "https://news.xbox.com", kind: "official", language: "英文" },
...
export function parseNews(xml: string, source: NewsSource, now = Date.now()): NewsArticle[] {
  ...
  for (const item of document.querySelectorAll("item,entry")) {
    ...
    const timestamp = Date.parse(item.querySelector("pubDate,published,updated")?.textContent || "");
    ...
    items.push({ id: url.href, title, url: url.href, publishedAt: new Date(timestamp).toISOString(), excerpt, sourceId: source.id, sourceName: source.name, sourceKind: source.kind, topic: /\brumou?r\b|\bleak(ed)?\b|传闻|爆料/i.test(title) ? "rumor" : /\binterview\b|访谈/i.test(title) ? "interview" : "news", language: source.language });
  }
  return items;
}
