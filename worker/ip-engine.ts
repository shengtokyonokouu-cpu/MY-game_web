...
export class IPEngine extends WorkerEntrypoint<Env> {
  async startScheduler() { if (this.env.IP_ENGINE_ENABLED !== "true") throw new Error("Cloudflare crawling disabled; use offline snapshot workflow"); return this.env.CLOCK.getByName("primary").start(); }
  ...
  async run(kind: string) {
    if (this.env.IP_ENGINE_ENABLED !== "true") return false;
...
export class IPEngineClock extends DurableObject<Env> {
  start() { if (this.env.IP_ENGINE_ENABLED !== "true") throw new Error("Scheduler disabled"); return this.ctx.blockConcurrencyWhile(() => armClock(this.ctx.storage)); }
  ...
  async alarm() {
    if (this.env.IP_ENGINE_ENABLED !== "true") { await pauseClock(this.ctx.storage); return; }
